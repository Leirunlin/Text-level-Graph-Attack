    ...
    for prefix in prefixs:
        ...
        if 'not_used' in files and 'used' in files:
            ...
            for id, (used_word, not_used_word) in tqdm(enumerate(zip(used_words, not_used_words))):
                if os.path.exists(f'LLAMA3/no_topic/{prefix}/result_{id}.txt'):
                    continue
                max_rate = 0
                final_use_rate = 0
                ...
                if use_rate1 >= max_rate:
                    ...
                    if not os.path.exists(f'LLAMA3/no_topic/{prefix}'):
                        os.makedirs(f'LLAMA3/no_topic/{prefix}')
                    with open(f'LLAMA3/no_topic/{prefix}/result_{id}.txt', 'w') as f:
                        f.write(Results)
