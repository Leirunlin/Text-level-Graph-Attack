        ...
        if 'not_used' in files and 'used' in files:
            ...
            for id, (used_word, not_used_word) in tqdm(enumerate(zip(used_words, not_used_words))):
                ...
                if use_rate1 >= max_rate:
                    ...
                    if not os.path.exists(f'GPT3.5/topic/{prefix}'):
                        os.makedirs(f'GPT3.5/topic/{prefix}')
                    with open(f'GPT3.5/topic/{prefix}/result_{id}.txt', 'w') as f:
                        f.write(Results)
